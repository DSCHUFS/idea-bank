...
exports.mypageAPI = async(req, res) => {
    try{
        ...
        for (let i = 0; i < pick_idea_id.length; i++) {
            let info = await res.pool.query(SELECT_DETAIL_PICK_INFO, [pick_idea_id[i]])
            info = info[0]
      ...
      pick_tmp = { ...info[0], idea_id: pick_idea_id[i] };
        ...
        let free = await res.pool.query(`SELECT COUNT(*) FROM PickIdeas WHERE user_id = ? AND pick_at = ?;`, [user_id, today])
